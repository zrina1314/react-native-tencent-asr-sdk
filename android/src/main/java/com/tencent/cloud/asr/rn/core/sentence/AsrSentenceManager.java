...
//            recognizer = new QCloudOneSentenceRecognizer(this,DemoConfig.apppId, "临时secretId", "临时secretKey","对应的token");
     ...
     * @param engSerViceType 转换语音的种类
     */
    public void recognizerFile(String filePath,String voiceFormat,String engSerViceType,Promise promise){
        this.promise = promise;
        try {
            String tempEngSerViceType = TextUtils.isEmpty(engSerViceType)?"16k_zh":engSerViceType;
            //Log.i(TAG,"tempEngSerViceType"+tempEngSerViceType);
...
//          params.setHotwordId(""); // 热词id。用于调用对应的热词表，如果在调用语音识别服务时，不进行单独的热词id设置，自动生效默认热词；如果进行了单独的热词id设置，那么将生效单独设置的热词id。
            ...
            params.setEngSerViceType(tempEngSerViceType); //默认16k_zh，更多引擎参数详见https://cloud.tencent.com/document/product/1093/35646 内的EngSerViceType字段
